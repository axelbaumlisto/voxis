// src/theme-engine/builtin/metaballs25d/index.ts
/**
 * Metaballs 2.5D — the "Metaballs 3D" look rebuilt on the 2D field engine.
 *
 * Same scalar metaball field + analytic-gradient coverage as `metaballs`
 * (pixel-perfect silhouette by construction, no raymarching, no WebGL), but
 * the albedo is a per-lobe colour mix: each blob owns one of the 3D theme's
 * four colours and the body colour at a pixel is the field-weighted blend of
 * the contributing lobes — exactly the i1*uCol1+i2*uCol2+... rule the 3D
 * shader uses, so fused necks melt colours the same way.
 *
...
const MAT = {
  ...
  // thin rim.
  // 2.5D: milder edge occlusion than the base 2D theme (0.48) — the 3D look
  // keeps lobe colour saturated right to the silhouette; a deep AO floor reads
  // as a dark membrane around the blob («плевра»).
  aoFloor: 0.86,
  aoRange: 0.14,
  ...
  // 2.5D: higher ambient floor (3D theme uses 0.30) so grazing angles never
  // crush to black at the rim.
  ambient: 0.38,
  lightStr: 0.85,
...
export function mount(container: HTMLElement, api: ThemeApi): ThemeInstance {
  ...
  // PER-LOBE COLOURS (the 3D theme's rule): each blob owns one colour from
  // `colors`; the pixel albedo is the field-weighted blend of contributing
  // lobes, so fused necks melt colours exactly like the raymarched version.
  // Defaults = the 3D theme's four lobe colours.
  const DEFAULT_PALETTE = ["#e63333", "#1a1ae6", "#33e633", "#e6e60d"];
  const isHex = (v: unknown): v is string =>
    typeof v === "string" && /^#?[0-9a-fA-F]{3,8}$/.test(v.trim());
  const validHex = Array.isArray(cfg.colors)
    ? (cfg.colors as unknown[]).filter(isHex)
    : [];
  ...
  const blobCount = Math.max(2, Math.min(8, Math.round(Number(cfg.blobCount) || 4)));
  ...
  // Per-blob colour (cycled through the palette), read in the field loop for
  // the weighted albedo blend. Fixed at mount — lobes keep their identity.
  const bcr = new Float64Array(blobCount);
  const bcg = new Float64Array(blobCount);
  const bcb = new Float64Array(blobCount);
  for (let i = 0; i < blobCount; i++) {
    const c = palette[i % nStops];
    bcr[i] = c.r;
    bcg[i] = c.g;
    bcb[i] = c.b;
  }
  ...
  function render(energy: number) {
    ...
    // Body-centroid setup for the shading dome (colour is per-lobe now — the
    // flowing gradient sweep of the base theme is gone; hue motion comes from
    // the lobes physically drifting).
    const gradCx = CW / 2;
    const gradCy = CH / 2;
    // Tighter inverse-radius for the SHADING dome, matched to the
    // real fused mass (~0.32 of min(CW,CH)) so the dome `dr` ramps to ~1.0 at the
    // actual silhouette (0.5 = half-canvas would cap dr at ~0.45 and never
    // round the body).
    ...
    const fld = { field: 0, gx: 0, gy: 0, cr: 0, cg: 0, cb: 0 };
    ...
    function sampleField(px: number, py: number): void {
      ...
      let cr = 0, cg = 0, cb = 0; // field-weighted per-lobe colour accumulators
      let wsum = 0;               // colour-weight normalizer (f² weights)
      for (let i = 0; i < blobCount; i++) {
        const dx = px - bx[i];
        const dy = py - by[i];
        ...
        gx += (-2 * rr * dx) / (d2 * d2);
        gy += (-2 * rr * dy) / (d2 * d2);
        // Colour weight = f^1.4: slightly peakier than the raw field (which
        // greys out mid-body where every lobe contributes) but far softer
        // than f², which made occluded lobes punch through the surface as
        // hard colour pucks (no occlusion in a 2D field).
        const wc = f * Math.sqrt(Math.sqrt(f * f * f));
        wsum += wc;
        cr += wc * bcr[i];
        cg += wc * bcg[i];
        cb += wc * bcb[i];
      }
      // Normalize the colour blend → convex weights (i1*c1+…), the same
      // per-lobe mixing rule the 3D shader applies at the surface.
      const inv = wsum > 1e-12 ? 1 / wsum : 0;
      fld.field = field; fld.gx = gx; fld.gy = gy;
      fld.cr = cr * inv; fld.cg = cg * inv; fld.cb = cb * inv;
    }
    ...
    function surfaceNormal(px: number, py: number): void {
      ...
      // horiz→1 → nz→0.
      // 2.5D: cap the in-plane roll at 0.72 so the normal never goes fully
      // sideways at the silhouette (nz stays ≥ ~0.69). A full roll (nz→0)
      // kills diffuse+AO right at the rim and reads as a dark shell wrapped
      // around the blob — the 3D theme's silhouette stays lit because its
      // ambient floor is additive per-lobe colour, not a multiplied AO.
      const ilen = Math.sqrt(ix * ix + iy * iy);
      const horiz = Math.min(0.72, ilen);
    ...
    function shadeGooey(px: number, py: number): void {
      ...
      // PER-LOBE ALBEDO (2.5D): the field-weighted blend of the contributing
      // lobes' colours, computed in sampleField — each lobe keeps its own hue
      // and fused necks melt between them, matching the 3D shader's
      // i1*uCol1+i2*uCol2+i3*uCol3+i4*uCol4 surface rule.
      const ar = fld.cr;
      const ag = fld.cg;
      const ab = fld.cb;
      ...
      // 2.5D: gentler Half-Lambert wrap (0.5/0.5 vs the base theme's 0.8/0.2)
      // — the away-from-light rim keeps ~half the key light, so the silhouette
      // stays colour-lit like the 3D theme instead of collapsing to a dark
      // shell around the blob.
      const w = Math.max(0, ndl * 0.5 + 0.5);
