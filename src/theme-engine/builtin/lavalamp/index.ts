// lavalamp — self-contained 3D raymarched lava lamp visualizer.
// The metaballs3d engine (smin-fused spheres, sphere-traced raymarch, the
// full edge-quality stack) with CONVECTION circulation instead of orbits:
// each wax blob rides a closed loop — rises on one side of the overlay,
// drifts across the top, sinks on the other side, drifts back along the
// bottom — like wax cycling in a lava lamp, adapted to the wide pill.
//
// Origin: metaballs3d + motion inspired by classic lava-lamp raymarch
// shaders (Shadertoy fsKXDm idiom: vertically cycling smin-fused spheres).
//
...
vec4 render(vec2 fragCoord) {
  ...
  float rad = 0.85 + 0.55 * uLevel;     // bigger wax blobs (spread-out loop)

  // LAVA-LAMP CONVECTION: each blob rides a closed elliptical circulation
  // loop around the pill — up one side, across the top, down the other side,
  // back along the bottom. An ellipse (wide X, shorter Y) traced at each
  // blob's own angular offset/speed IS a convection cell for a horizontal
  // vessel; per-blob wobble (second harmonic at an irrational ratio) breaks
  // the mechanical look so blobs meet, fuse and separate like real wax.
  // X half-span uses the aspect so the loop fills the wide pill.
  // Loop size: the visible half-span at z=5 is ~2.5*uZoom*aspect horizontally
  // and ~2.5*uZoom vertically (perspective at unit focal). Fill ~55% of it so
  // blobs sweep the whole pill but never clip the frame.
  float spanX = 1.4 * uZoom * aspect;
  float spanY = 1.15 * uZoom;
  vec3 c1 = vec3( spanX*cos(uPhase),
                  spanY*sin(uPhase)             + 0.35*sin(2.7*uPhase), 5.0);
  vec3 c2 = vec3( spanX*cos(uPhase + 1.57),
                  spanY*0.92*sin(uPhase + 1.57) + 0.40*sin(2.3*uPhase), 5.0);
  vec3 c3 = vec3( spanX*cos(0.8*uPhase + 3.14),
                  spanY*sin(0.8*uPhase + 3.14)  + 0.30*sin(3.1*uPhase), 5.0);
  vec3 c4 = vec3( spanX*cos(1.15*uPhase + 4.71),
                  spanY*0.85*sin(1.15*uPhase + 4.71) + 0.45*sin(2.0*uPhase), 5.0);
...
function compile(gl: WebGLRenderingContext, type: number, src: string): WebGLShader {
  ...
  if (!gl.getShaderParameter(sh, gl.COMPILE_STATUS)) {
    ...
    gl.deleteShader(sh);
    throw new Error("lavalamp shader compile failed: " + log);
  }
  return sh;
}
...
export function mount(container: HTMLElement, api: ThemeApi): ThemeInstance {
  ...
  // EXACTLY 2x: the compositor downscales the canvas layer with plain bilinear
  // (2x2 taps). At 2:1 with aligned grids bilinear degenerates into an exact
  // box filter (every source pixel contributes once) — clean AA. At 3:1 it
  // SKIPS 5 of every 9 supersampled pixels and the staircase survives.
  // Ref: bartwronski.com bilinear down/upsampling; KhronosGroup/WebGL#3360.
  const renderScale = 2;
  ...
  function bail(reason: string): ThemeInstance {
    console.warn("lavalamp: WebGL unavailable, skipping", reason);
    canvas.remove();
    return { unmount() {} };
  }

  // premultipliedAlpha:true + shader outputs premultiplied rgb: filtering and
  // compositing only commute under premultiplied alpha (IQ “premultiplied
  // alpha” article) — straight alpha gets RGB interpolated independently of A
  // during the compositor's downscale/blend, fringing the silhouette.
  const gl = canvas.getContext("webgl", { premultipliedAlpha: true, alpha: true }) as
