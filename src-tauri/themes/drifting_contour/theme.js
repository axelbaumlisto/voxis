...
function runIntentHeading(context, stimulus, photoAdapt) {
  ...
  const comfortDrive = 0.8;
  return wrapPi2(base + wrapPi2(bearing - base) * (comfortDrive + 0.18 * lowDrive));
}
  ...
  return euglena.map((cell, idx) => {
    ...
    let motorOutput;
    {
      ...
      if (!motorEnabled && photoIntent > 0 && safeWidth > 0 && safeHeight > 0) {
        ...
        const eyeGate = 0.76 + 0.24 * Math.max(0, Math.cos(bearing - heading - TAU2 * finite(cell.rollPhase, 0)));
...
// src/theme-engine/renderers/cell/aquarium/vorticella-parts/behaviour.ts
...
function drawVorticella(ctx, vorticella, frame, view) {
  ...
  for (const cell of vorticella) {
    ...
    const arrestT = vleg === 2 ? Math.max(0, finiteOr(cell.contractTimer, 0)) : vleg === 3 ? T_HOLD2 + Math.max(0, finiteOr(cell.contractTimer, 0)) : -1;
