...
function drawVorticella(ctx, vorticella, frame, view) {
  ...
  for (const cell of vorticella) {
    ...
    const asymA = (seededUnit(aSeed, 0, 17) - 0.5) * 0.14;
    ...
    const geom = vorticellaGeometry(s, {
      ...
      minLengthFrac: 0.32,
      coilSampleCount: 30,
      coilTurnsContracted: 3,
      coilRadius: D * 0.24
    });
    const neck = geom.bellCenter;
    const rimC = { x: neck.x + ux * bellHeight + nx * periOff * D, y: neck.y + uy * bellHeight + ny * periOff * D };
    const open = 1 - 0.7 * s;
    ...
    for (let k = 0;k < gCount; k++) {
      ...
      ctx.fillStyle = seededUnit(gSeed, k, 10293743) > 0.5 ? `hsla(48, 14%, 94%, ${alpha * 0.2})` : `hsla(210, 12%, 38%, ${alpha * 0.18})`;
      ctx.fill();
    }
    ...
    for (let k = 0;k < gCount2; k++) {
      ...
      ctx.fillStyle = seededUnit(gSeed, k, 10031565) > 0.5 ? `hsla(46, 12%, 90%, ${alpha * 0.12})` : `hsla(208, 12%, 42%, ${alpha * 0.12})`;
      ctx.fill();
    }
