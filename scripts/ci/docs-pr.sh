...
# Vite (`bun run dev`) INSIDE the docs container against this clone (bind-mounted
# at /w); without deps Vite never comes up, so the screenshotter silently
# produces NO images and only the docs-auditor's text edits land in the PR
# (observed: PR had .md changes only, PNGs stayed stale). Install deps here so
# node_modules exists in the mounted clone. The Playwright browser is NOT
# installed here — it is baked into the voxis-docs image (/opt/ms-playwright).
...
# Run the docs agents (screenshotter + auditor) via the containerized runner:
# each agent runs inside an ephemeral voxis-docs container (pi + Vite +
# Playwright all INSIDE it), torn down with `docker rm -f` — nothing leaks to
# the shared host. Non-fatal.
