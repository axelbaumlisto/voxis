...
ротируются, но JSONL дописывается до явной очистки отладочных данных; это может
увеличить расход диска и раскрыть содержимое записей. Отключайте debug при
