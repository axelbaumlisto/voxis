...
- Provider (`cloud_provider`): Groq or OpenAI in the UI. This label is stored in config and debug metadata, but the current transcription client does not route by it; it uses the default Groq-compatible transcription URL unless the custom endpoint field below is set.
...
- Custom Endpoint URL (Advanced) (`api_url_override`): optional OpenAI-compatible `/audio/transcriptions` endpoint. Leave it blank for Groq. For local and alternative endpoints, see [Self-Hosted Transcription](self-hosted-transcription.md).
